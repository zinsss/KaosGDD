...
for command in dpkg-divert nsenter rsync sha256sum systemctl; do
...
"$FAXMAIL/install-telegram-faxrcvd.sh" --install
...
systemctl disable --now kaos-faxmail-retry.timer 2>/dev/null || true
rm -f \
  /etc/systemd/system/kaos-faxmail-retry.service \
  /etc/systemd/system/kaos-faxmail-retry.timer \
  /etc/logrotate.d/kaosgdd-faxmail \
  /etc/kaosgdd/faxmail.env \
  /usr/local/lib/kaosgdd/faxmail/send-incoming-fax-email.py
rm -rf /var/spool/hylafax/status/kaosgdd-faxmail
